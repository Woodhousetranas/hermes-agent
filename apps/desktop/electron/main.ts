...
function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    root,
    // Native Windows starts can finish their one-shot stdout announcement
    // while the ownership claim is still validating process identity. Use
    // the backend's atomic ready-file channel so the port cannot be missed.
    readyFile: IS_WINDOWS,
...
function createActiveBackend(backendArgs) {
  ...
  return {
    ...
    // Keep packaged Windows startup independent of stdout pipe timing.
    readyFile: IS_WINDOWS,
    bootstrap: true,
    shell: false
  }
