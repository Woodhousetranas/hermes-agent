    ...
    * anything else — run with the current Python interpreter
    ...
    try:
        if script_timeout_seconds is None:
            # Preserve the no-argument call shape for existing monkeypatches
            # and integrations that override the global timeout resolver.
    ...
    try:
        ...
        if suffix in {".sh", ".bash"} and sys.platform == "win32":
            ...
            # tools. The generic terminal env disables MSYS path conversion;
            # trusted cron scripts need the native conversion behavior.
        ...
        try:
            ...
            if should_deliver and delivery_error is None and confirm_script:
                ...
                if job.get("script_timeout_seconds") is not None:
                    confirm_kwargs["script_timeout_seconds"] = job["script_timeout_seconds"]
                confirm_ok, confirm_output = _run_job_script(confirm_script, **confirm_kwargs)
                if not confirm_ok:
                    delivery_error = f"delivery confirm script failed: {confirm_output}"
                    logger.error(
                        "Delivery confirm failed for job %s: %s",
                        job["id"],
                        confirm_output,
                    )
