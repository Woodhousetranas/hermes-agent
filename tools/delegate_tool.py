...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "BOTH MODES RUN IN THE BACKGROUND (background=true). delegate_task "
        "returns a delegation_id immediately - you and the user keep working, "
        "and each subagent's full result "
        ...
        "- Durable scheduled/retryable work or Portal-owned operations -> "
        "use cronjob (action='create'), Portal/Work Queue contracts, "
        "or terminal(background=True, notify_on_complete=True) instead. "
        "Background delegations are not a durable job queue: if the parent "
        "session is closed (/new) or the process exits before a subagent "
        "finishes, that subagent's work is discarded, and /stop cancels "
        "every running background subagent.\n\n"
        "IMPORTANT:\n"
